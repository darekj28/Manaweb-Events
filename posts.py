...
class Posts:
	def __init__(self):
		...
		self.DELETED_POST_MESSAGE = "[deleted]"
		self.DELETED_COMMENT_MESSAGE = "[deleted]"
		self.DELETED_ACCOUNT_AVATAR = "rip"
		self.DELETED_USER_ID = ""
		self.DELETED_ACCOUNT_FIRST_NAME = "[deleted]"
		self.DELETED_ACCOUNT_LAST_NAME = ""
	...
	def sendNotification(self, feed_name, comment_id, receiver_id, sender_id, original_post) :
		...
		pusher.trigger('notifications','new_notification_for_' + receiver_id.lower(), {'data' : ""})
	...
	def createShortList(self, receiver_id):
		...
		sql = 'CREATE TABLE IF NOT EXISTS ' + table_name + ' (feed_name TEXT, comment_id TEXT, \
			receiver_id TEXT, sender_id TEXT, action TEXT, seen BOOLEAN, notification_id TEXT, timeStamp FLOAT, timeString TEXT, \
			numUnseenActions INTEGER, sender_name TEXT, op_name TEXT, numOtherPeople INTEGER, isOP BOOLEAN, pushNotificationSent BOOLEAN)'
		self.db.execute(sql)
		addIndexCode = 'CREATE INDEX IF NOT EXISTS comment_id ON ' + self.NOTIFICATION_TABLE + ' (comment_id)'
		self.db.execute(addIndexCode)
		addIndexCode = 'CREATE INDEX IF NOT EXISTS receiver_id ON ' + self.NOTIFICATION_TABLE + ' (receiver_id)'
	...
	def markPushNotificationsAsSent(self, userID):
		sql = "UPDATE " + self.USER_NOTIFICATION_PREFIX + userID + " SET pushNotificationSent = %s"
		pushNotificationSent = True
		self.db.execute(self.db.mogrify(sql, (pushNotificationSent,)))
		self.post_db.commit()

	def getPushNotifications(self, userID):
	...
	def markNotificationAsSeen(self, userID):
		...
		self.db.execute(self.db.mogrify(sql, (seen,)))
		self.post_db.commit()
		# if the notificaitons are seen then so are the notifications
		self.markPushNotificationsAsSent(userID)
	...
	def reportQueryToDict(self, query_item):
		...
		report_dict['unique_id'] = query_item[1]
	...
	def reportComment(self, feed_name, comment_id, unique_id, reason, description, reporting_user, reported_user):
		...
		timeStamp = time.time()
		timeString = self.getTimeString(timeStamp)
		self.db.execute(self.db.mogrify("INSERT INTO " + self.REPORT_TABLE + "(feed_name, comment_id, unique_id, body, reason, isComment, description, timeStamp, timeString, reporting_user, reported_user) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)", (feed_name, comment_id, unique_id, body, reason, True, description, timeStamp, timeString, reporting_user, reported_user)))
		self.post_db.commit()	
	...
	def addColumn(self, table_name, column_name, column_type, default_value = None):
		...
		if (default_value != None):
			sql = "UPDATE " + table_name + " SET " + column_name + " = %s"
			self.db.execute(self.db.mogrify(sql, (default_value,)))
